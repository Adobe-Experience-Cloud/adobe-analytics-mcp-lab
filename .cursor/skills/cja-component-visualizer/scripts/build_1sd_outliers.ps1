# Build network with statistical outliers (Mean + 1.0 SD per category)
...
Write-Host "Building network with statistical outliers (Mean + 1.0 SD)..." -ForegroundColor Cyan
...
foreach ($type in @('dimension', 'metric', 'calculatedMetric', 'segment')) {
    ...
    foreach ($comp in $selected) {
        Write-Host "    - $($comp.Name.Substring(0, [Math]::Min(60, $comp.Name.Length))): usage = $($comp.Value.usage)" -ForegroundColor Gray
        $selectedComponents += $comp.Name
    }
}

Write-Host "`nTotal selected: $($selectedComponents.Count) statistical outliers (Mean + 1.0 SD)" -ForegroundColor Green
...
# Save results
$selectedComponents | ConvertTo-Json | Out-File (SamplePath 'statistical_1sd_components.json') -Encoding UTF8
$uniqueConnections | ConvertTo-Json | Out-File (SamplePath 'network_1sd_connections.json') -Encoding UTF8

Write-Host "`nSaved:" -ForegroundColor Green
Write-Host "  synthetic_sample/statistical_1sd_components.json"
Write-Host "  synthetic_sample/network_1sd_connections.json"
